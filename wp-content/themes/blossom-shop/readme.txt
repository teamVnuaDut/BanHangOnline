=== Blossom Shop ===

Contributors: blossomthemes
Requires at least: 4.7
Requires PHP: 5.6
Tested up to: 6.5
Stable tag: 1.2.1
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
Tags: blog, one-column, two-columns, right-sidebar, left-sidebar, footer-widgets, custom-background, custom-header, custom-menu, custom-logo, featured-images, threaded-comments, full-width-template, rtl-language-support, translation-ready, theme-options, e-commerce, portfolio

== Description ==
Blossom Shop is a clean, fast and feature-rich free WordPress theme to create online stores. It is powered by WooCommerce plugin. You can use it to build all kinds of online shop such as fashion, cosmetics, jewelry, furniture, accessories, lifestyle, decoration, gadgets, books, home appliances, clothes, automobiles, movies, and so so. Blossom Shop is designed to highlight products and provide user experience of the highest quality so that your products are sold easily like candy. You can easily change the look and feel of your website using the color and typography features. You can change the color to purple, pink, cyan or any color you like. The theme is mobile friendly and fully responsive to ensure your website looks great in different sized devices. Beautifully placed Instagram section can be used to show your Instagram account and increase your followers. Blossom Shop is Schema.org ready, SEO friendly and speed optimized so that your website ranks over your competitors. It is multilingual ready and supports RTL. Other features include social sharing, Instagram profile, newsletter subscription, similar products, sales ribbon and Mega Menu compatible. In addition, Blossom Shop is backed by world class, one-on-one support, and a super active team of developers and designers ready to help! Check theme details at https://blossomthemes.com/downloads/blossom-shop-free-wordpress-theme/, demo at https://blossomthemes.com/theme-demo/?theme=blossom-shop, read the documentation at https://docs.blossomthemes.com/docs/blossom-shop/, and get support at https://blossomthemes.com/support-ticket/.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Copyright ==

Blossom Shop WordPress Theme, Copyright 2017 blossomthemes.com
Blossom Shop is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Blossom Shop bundles the following third-party resources:

Owl-carousel, Copyright 2013-2017 David Deutsch 
License : MIT
Source  : https://owlcarousel2.github.io/OwlCarousel2

Owl Carousel v2 Accessibility Layer, Copyright 2016 Geoffrey Roberts
License : MIT
Source  : https://github.com/rtrvrtg/owlcarousel2-a11ylayer

animate.css, Copyright (c) 2017 Daniel Eden
License: MIT
Source: https://github.com/daneden/animate.css

normalize.css, Copyright 2012-2016 Nicolas Gallagher and Jonathan Neal
License: MIT
Source: https://necolas.github.io/normalize.css/

Font Awesome icons, Copyright Dave Gandy
Icons License: CC BY 4.0 
Fonts License: SIL OFL 1.1
Source: http://fontawesome.io/

Selectize, Copyright � 2013�2016 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/selectize/selectize.js

Sifter, Copyright � 2013�2015 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/sifter.js/

Microplugin, Copyright � 2013 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/microplugin.js

TGMPA, Copyright (c) 2011, Thomas Griffin
License: GPL-2.0+
Source: http://tgmpluginactivation.com/

Blossom Shop Theme incorporates code from Kirki, Copyright (c) 2017 Aristeides Stathopoulos
License: https://github.com/aristath/kirki/blob/develop/LICENSE
Source: https://github.com/aristath/kirki

Blossom Shop local fonts feature is based on Webfonts Loader by WordPress Themes Team (WPTT). (https://github.com/WPTT/webfont-loader)
License: MIT
Source: https://github.com/WPTT/webfont-loader/blob/master/LICENSE

Some codes are taken from https://blog.josemcastaneda.com/2016/08/08/copy-paste-hurting-theme/

Some codes are taken from Twenty Twenty theme - https://wordpress.org/themes/twentytwenty/

Banner Image 		
Source  : https://pxhere.com/en/photo/112362
License : CC0 Public Domain, https://pxhere.com/en/license

404 page Image 
Source  : https://stocksnap.io/photo/PHOFG380TU
License : CC0 Public Domain, https://stocksnap.io/license

Page Header Image
Source  : https://pxhere.com/en/photo/1433619
License : CC0 Public Domain, https://pxhere.com/en/license

All other images including icon image used in screenshot and images used in customizer settings are self created and are under the GPL.

== Changelog ==

    1.2.1 - June 06, 2024
        * Update: New dashboard has been implemented for better user experience.
        * Update: Google Fonts Library has been updated.

    1.2.0 - May 20, 2024
        * Fix: Minor refinements have been done in the dashboard links.
        * Update: Google Fonts Library has been updated.

    1.1.9 - February 08, 2024
        * Update: Issue with ajax requests has been fixed.
        * Update: Google Fonts Library has been updated.

    1.1.8 - February 01, 2024
        * Update: Added nonce to the custom admin notice to avoid CSRF vulnerability.
        * Update: Google Fonts Library has been updated.

    1.1.7 - December 11, 2023
        * Fix: Design issue of single product gallery has been fixed for the WooCommerce version  8.3.1.
        * Update: Google Fonts Library has been updated.

    1.1.6 - November 2, 2023
        * Update: Google Fonts Library has been updated.
        * Update: Now users can add Instagram shortcode from any plugins of their choice

    1.1.5 - July 24, 2023
        * Fix: Warning shown for php 8 users while installing plugins through TGMPA has been fixed.
        * Update: Google Fonts Library has been updated.
        * Update: Relevant tags for the theme have been added.  

    1.1.4 - July 14, 2022
        * Added: feature to load google font locally

    1.1.3 - November 11, 2021
        * Added: Feature for banner caption alignment
        * Added: Added product categories in featured section
        * Fixed: Design fixes for sub-menu and featured section product categories

    1.1.2 - July 21, 2021
        * Fixed: jQuery deprecated warnings
        * Added: link underline in post/page
        * Updated: Google Fonts
        * Removed block widgets for wordpress 5.8
    
    1.1.1 - December 25, 2020
        * Fixed: customizer load issue

    1.1.0 - November 26, 2020
        * Modified: made content links accessible

    1.0.9 - November 13, 2020
        * Added: feature to control automatic cropping of product images on homepage sections

    1.0.8 - September 24, 2020
        * Fixed: social media settings icon issue
        * Fixed: slider control value not showing issue
        * Modified: theme homepage link

    1.0.7 - April 30, 2020
        * Fixed: Calendar widget design issue for WordPress 5.4.1
        * Fixed: Menu focus issue while last item has submenu
        * Fixed: Page header spacing issue
        * Fixed: Translation issue
        * Added: Overlay in page header
        * Added: Feature to upload page header background image
        * Changed: Text color in page header
	
    1.0.6 - April 21, 2020
        * Fixed: Some design issues
        * Improved: Changed screenshot and description
    
    1.0.5 - April 17, 2020
        * Fixed: Search popup accessibility issue
        * Fixed: Responsive menu accessibility issue
        * Fixed: Other issues listed from theme reviewer 

    1.0.4 - April 7, 2020
        * Fixed: Issues listed from theme reviewer

    1.0.3 - March 18, 2020
        * Added: Owl Carousel v2 Accessibility Layer js
        * Fixed: instagram section design issue
        * Improved: adding inline style

	1.0.2 - January 13, 2020
        * Fixed: Typo error
        * Fixed: Edge hover issue
        * Changed: Getting Started support description
            
    1.0.1 - October 22, 2019
        * Fixed: Breadcrumb Error
        * Fixed: Multiple woocommerce category desc
        * Tweaked: Updated Google Fonts library
        
    1.0.0
        * Initial release